...
This file is intended to be copied into (or referenced from) another project's `CLAUDE.md` so Claude can implement operational logging using `pocketlogR`.
...
`pocketlogR` logs application events (ETL jobs, website checks, email confirmations, database freshness checks) to a shared PocketBase instance. It supports DAG-based flow dependencies so you can see cascading failures across pipelines at a glance.
...
These must be set in `.Renviron` or the system environment. **Never hardcode them.**
...
`pl_connect()` picks these up automatically when called with no arguments.
...
Logging functions (`pl_success`, `pl_error`, `pl_fatal`, `pl_log`) **never stop the calling script** — they retry 3 times on failure, then emit a warning and return NULL invisibly.

---

## Registering a flow (one-time)

Before logging, the flow must exist in PocketBase. This only needs to be done once:
...
`owner` is required. `metadata` is an optional named R list serialised to JSON — use it for any flow-level context (URLs, contacts, config, tags).

`pl_create_flow()` errors if the flow already exists — safe to guard with a check:
...
Declare upstream dependencies so `pl_get_dag()` can show cascading failures:
...
- Do not call `pl_setup()` — the collections already exist in PocketBase.
- Do not call `pl_connect_admin()` — superuser access is not needed for logging.
- Do not wrap `pl_success` / `pl_error` in additional `tryCatch` — they already handle retries internally and will never throw.
- Do not log inside a tight loop — log once per job run (start/end), not per row.
